#!/usr/bin/env python3
"""Same as char4_shuffle_critic.py but reads character_4 assets + metadata from a private,
static snapshot directory instead of the live repo -- the live repo's character_4 assets
(head/eyes/body/metadata.json) were observed being repeatedly overwritten by some external
process mid-session (never the mouth files themselves, but metadata.json IS touched, which
can corrupt the composited render even though the mouth PNGs on disk are correct). This
script decouples verification rendering from that churn.

Usage: ../.venv/bin/python tools/char4_shuffle_critic_isolated.py happy round4 /tmp/c4_snapshot
"""
import json
import os
import random
...
def headzoom(meta, char, im, base):
    m = meta[char]
    explain_dir = f"{base}/{char}/body/explain"
    bfile = sorted(os.listdir(explain_dir))[0]
    stem = os.path.splitext(bfile)[0]
    b0 = m["body"][stem]
    white_dir = f"{base}/{char}/background/white"
...
def main():
    ...
    base = sys.argv[3] if len(sys.argv) > 3 else "/tmp/c4_snapshot"
    char = "character_4"
    suf = "h" if mood == "happy" else "s"
    out_dir = f"build/qa/character_4/critic_{round_name}_{mood}"
    os.makedirs(out_dir, exist_ok=True)

    mgr = CharacterManager(base, f"{base}/metadata.json")
    ...
    for vis in VISEMES:
        ...
        z = headzoom(meta, "character_1", im, base)
